...
\<The mutate function creates a new variable capita_kg_year and also organises the income category levels from HIC to LIC\>
...
\<The mutate function enabled the creation of the new variable of MSW generation per capita which makes it easier to compare countries of varying populations\>

5.  Describe in your own words what the 'color' argument inside the `aes()` does

\<Color argument helps to distingush between the different income levels on the plot by assigning specific colours to each income group\>

6.  Describe in your own words what the `labs()` function does

\<Labs is used to label the x and y axis on the plot\>
